...
module.exports = {
  ...
  delete: function(req, res, next) {
    ...
    apps.get(appname, function(err, doc) {
      var app_user_home = path.join(config.opt.apps_home_dir, user._id, app.repo_id);
      var app_git_home = path.join(config.opt.git_home_dir, user._id, app.repo_id);
      if (err) {
        res.writeHead(500, {
          ...
          if (err) {
            ...
            lib.update_proxytable_map(function(err) {
              app_stop(app.repo_id, function() {
                exec(config.opt.app_dir + '/scripts/removeapp.js ' + app_user_home + ' ' + app_git_home, function() {
                  res.end(JSON.stringify({
                    "status": "success"
                  }));
                });
  ...
  post: function(req, res, next) {
      ...
      if (!coupon || coupon != config.opt.coupon_code) {
	    res.send({
		status: "failure",
		message: "Coupon code invalid or already used"
	    });
	    return;
      }
      var user = req.user;
                            ...
                            apps.save(appname, {
                                ...
                                if (err) {
                          		...
                          		exec(cmd, function(err, stdout, stderr) {
                                        ...
                                        //  var gitsetup = spawn(config.opt.app_dir + '/scripts/gitreposetup.sh', [config.opt.app_dir, config.opt.apps_home_dir, user._id, repo_id, start]);
